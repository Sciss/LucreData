...
//      new Impl[ B, A ]( minGap, key maxKey )
   def empty[ A : mutable.Ordering : MaxKey : Manifest ] : HASkipList[ A ] = empty()
   def empty[ A ]( minGap: Int = 2, keyObserver: SkipList.KeyObserver[ A ] = SkipList.NoKeyObserver )
                 ( implicit ord: mutable.Ordering[ A ], maxKey: MaxKey[ A ], mf: Manifest[ A ]) : HASkipList[ A ] = {
   ...
   private final class Impl[ /* @specialized( Int, Long ) */ A ]
      ...
      ( implicit mf: Manifest[ A ], val ordering: mutable.Ordering[ A ])
