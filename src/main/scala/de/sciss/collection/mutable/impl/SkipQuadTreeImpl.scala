...
import de.sciss.collection.geom.{Dim, QueryShape, Point2DLike, DistanceMeasure}
...
trait SkipQuadtreeImpl[ A ] extends SkipQuadtree[ A ] {
   ...
   final def rangeQuery( qs: QueryShape[ Dim.Two ]) : Iterator[ A ] = new RangeQuery( qs )
...
//   protected def nn( point: Point2DLike, metric: DistanceMeasure ) : QLeaf
   ...
   private final class RangeQuery( qs: QueryShape[ Dim.Two ]) extends Iterator[ A ] {
      ...
      def rangeQueryLeft( node: QNode, area: Long, qs: QueryShape[ Dim.Two ]) : QNode = {
         var i = 0; while( i < 4 ) {
            node.child( i ) match {
               case n2: QNode =>
      ...
      def rangeQueryRight( node: QNode, area: Long, qs: QueryShape[ Dim.Two ]) : QNode = {
         var i = 0; while( i < 4 ) {
            node.child( i ) match {
               case n2: QNode =>
                  val a2 = qs.overlapArea( n2.quad )
