...
//         } else {
            val tree = txn.DeterministicSkipOctree.empty[ InMemory, Space.TwoDim, Point2DLike ](
               Space.TwoDim, Square( sz, sz, sz ), skipGap = 1 )
...
//      f.setResizable( false )
   ...
   private final class Model2D[ S <: Sys[ S ]]( val tree: txn.DeterministicSkipOctree[ S, Space.TwoDim, Point2DLike ])
   extends Model[ S, Space.TwoDim ] {
//      val tree = DeterministicSkipOctree.empty[ S, Space.TwoDim, Point2DLike ]( Space.TwoDim, Square( sz, sz, sz ), skipGap = 1 )
      ...
      val view = {
         val res = new TxnSkipQuadtreeView[ S, Point2DLike ]( tree )
...
//      val baseDistance = DistanceMeasure2D.euclideanSq
      ...
      def addPDFSupport( f: JFrame ) {
         PDFSupport.addMenu[ TxnSkipQuadtreeView[ S, Point2DLike ]]( f, view :: Nil, _.adjustPreferredSize() )
      }
   }
