...
object DeterministicSkipOctree {
   def empty[ S <: Sys[ S ], D <: Space[ D ], A ]( space: D, hyperCube: D#HyperCube, skipGap: Int = 2 )
      ...
      new Impl[ S, D, A ]( space, hyperCube, view, { implicit impl =>
         ...
         val headRight     = system.newRef[ RightNode[ S, D, A ]]()
         val head          = new TopLeftNode[ S, D, A ]( hyperCube, order.root, children, headRight )
         val headM         = system.newMut[ TopNode[ S, D, A ]]( head )
         val lastTreeRef   = system.newRef[ TopNode[ S, D, A ]]( headM )
...
//   }
   ...
   private type Order[ S <: Sys[ S ]] = S#Mut[ TotalOrder.SetEntry[ S ]]
   ...
   private final class TopRightNodeSer[ S <: Sys[ S ], D <: Space[ D ], A ]
   extends Serializer[ TopRightNode[ S, D, A ]] {
      def read( in: DataInput ) : TopRightNode[ S, D, A ] = sys.error( "TODO" )
      def write( node: TopRightNode[ S, D, A ], out: DataOutput ) {
         sys.error( "TODO" )
      }
   }
   ...
   with Serializer[ Leaf[ S, D, A ]] {
      ...
      implicit val topRightNodeSer: Serializer[ TopRightNode[ S, D, A ]] = new TopRightNodeSer[ S, D, A ]
...
//      def lastTree : Node = lastTree
      ...
      def lastTree( implicit tx: S#Tx ) : TopNode[ S, D, A ] = lastTreeRef.get.get
      def lastTree_=( node: S#Mut[ TopNode[ S, D, A ]])( implicit tx: S#Tx ) {
...
//         order.compare( that.order )
         val t1 = a.order.get.tag
         val t2 = b.order.get.tag
...
//println( "up : " + l )
         ...
         val pNext      = if( pNext0 == null ) { // create new level
            ...
            val nextRef = system.newRef[ RightNode[ S, D, A ]]()
            val prev    = lastTree
            val res     = new TopRightNode( hyperCube, prev, ch, nextRef )
            val resM    = system.newMut[ TopRightNode[ S, D, A ]]( res )
            prev.next   = resM
            lastTree    = resM
            res
         } else pNext0
...
//      findNextValue()
   ...
   extends LeftNonEmpty[ S, D, A ] with InnerNonEmpty[ S, D, A ] with LeftChild[ S, D, A ] {
      private[DeterministicSkipOctree] def parentLeft_=( p: S#Mut[ LeftNode[ S, D, A ]])( implicit tx: S#Tx ) : Unit
   ...
   extends InnerNonEmpty[ S, D, A ] with RightChild[ S, D, A ] {
      private[DeterministicSkipOctree] def parentRight_=( p: S#Mut[ RightNode[ S, D, A ]])( implicit tx: S#Tx ) : Unit
...
//      private var parentVar: Node[ S, D, A ] = null
      ...
      private[DeterministicSkipOctree] def parentLeft_=( p: S#Mut[ LeftNode[ S, D, A ]])( implicit tx: S#Tx )   { parent_=( p )}
      private[DeterministicSkipOctree] def parentRight_=( p: S#Mut[ RightNode[ S, D, A ]])( implicit tx: S#Tx ) { parent_=( p )}
      private[DeterministicSkipOctree] def parent( implicit tx: S#Tx ): Node[ S, D, A ] = parentRef.get.get
      private[DeterministicSkipOctree] def parent_=( p: S#Mut[ Node[ S, D, A ]])( implicit tx: S#Tx ) { parentRef.set( p )}
...
//         order.remove()
   ...
   extends NonEmpty[ S, D, A ] with Disposable[ S#Tx ] /* with QNode */ {
      /**
       ...
       * node exists.
       */
      final private[DeterministicSkipOctree] def next( implicit tx: S#Tx ) : RightNode[ S, D, A ] = nextRef.get.get
       ...
       * node in Qi+1.
       */
      final private[DeterministicSkipOctree] def next_=( node: S#Mut[ RightNode[ S, D, A ]])( implicit tx: S#Tx ) {
         nextRef.set( node )
      }
...
//      protected def nextRef: S#Ref[ RightNode[ S, D, A ]]
      ...
      private[DeterministicSkipOctree] final def insert( leaf: Leaf[ S, D, A ])
         ...
         if( c.isEmpty ) {
            ...
            val n2M     = newNode( qidx, pPrev, qn2 )
            ...
            if( old.parent == this ) old.parentRight_=( n2M )
...
//         l.parent = this
      ...
      @inline private def newNode( qidx: Int, iq: D#HyperCube )
                                 ( implicit tx: S#Tx, impl: Impl[ S, D, A ]) : S#Mut[ InnerLeftNode[ S, D, A ]] = {
