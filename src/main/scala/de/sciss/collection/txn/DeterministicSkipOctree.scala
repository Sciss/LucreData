...
//println( "down : " + l )
   ...
   private final class NN[ S <: Sys[ S ], D <: Space[ D ], @specialized( Int, Long ) A, @specialized( Long ) M ](
...
//            if( space.bigGt( acceptedChildren( j ).minDist, rmax )) { ... }
      ...
      @tailrec def findNNTail( n0: Node[ S, D, A ]) {
...
//println( "      : node " + cq + " " + identify( c ) + " - " + cMaxDist )
         ...
         while( succ != null ) {
            succ.child( accept1Idx ) match {
               case dn2: Node[ S, D, A ] if( dn2.hyperCube == qdn ) =>
                  dn    = dn2
                  succ  = succ.next
               case _ =>
                  succ  = null
         ...
         // now go left
         @tailrec def findLeft( n: Node[ S, D, A ]) : LeftNode[ S, D, A ] = {
            n match {
               case l: LeftNode[ S, D, A ] => l
               case r: RightNode[ S, D, A ] => findLeft( r.prev )
            }
         }
...
//            while( space.bigGt( vis.minDist, rmax ))
   ...
   private final class VisitedNode[ S <: Sys[ S ], D <: Space[ D ], @specialized( Int, Long ) A, @specialized( Long ) M ]
...
//      def compare( that: VisitedNode[ S, D, A, M ]) = metric.compareMeasure( that.minDist, minDist )
   ...
   ( qs: QueryShape[ Area, D ])( implicit impl: Impl[ S, D, A ]) extends Iterator[ A ] {
      ...
      @tailrec def findEquiStabbingTail( node: Node[ S, D, A ], area: Area ) : Node[ S, D, A ] = {
         ...
         pi match {
            case l: LeftNode[ S, D, A ]  => l
            case r: RightNode[ S, D, A ] => findEquiStabbingTail( r.prev, area )
         }
      }
      ...
      def findHighestUncritical( p0: Node[ S, D, A ], area: Area ) : Node[ S, D, A ] = {
         var pi         = p0.next
      ...
      def findNextValue() { while( true ) {
         if( in.isEmpty ) {
            ...
            case n: Node[ S, D, A ] =>
               var i = 0; while( i < sz ) {
                  n.child( i ) match {
                     case ne: NonEmpty[ S, D, A ] => in += ne   // sucky `enqueue` creates intermediate Seq because of varargs
                     case _ =>
                  }
               i += 1 }
         }
