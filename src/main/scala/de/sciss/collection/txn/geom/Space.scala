...
import geom.{Point2D, Point3D, Square, Cube}


object Space {
   implicit object Point2DSerializer extends Serializer[ geom.Point2D ] {
      def read( in: DataInput ) : Point2D = {
         val x = in.readInt()
         val y = in.readInt()
         Point2D( x, y )
      }

      def write( p: Point2D, out: DataOutput ) {
         out.writeInt( p.x )
         out.writeInt( p.y )
      }
   }
