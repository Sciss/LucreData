...
class AncestorRetroSuite extends FeatureSpec with GivenWhenThen {
   ...
   abstract class AbstractTree[ A ]( _init: A ) {
      ...
      implicit val vertexSerializer : Serializer[ V ]
...
//         val preTail = pre.append() // insertAfter( () )
      ...
      def insertChild( parent: Vertex, value : A )( implicit tx: S#Tx ) : Vertex = {
...
if( verbose ) println( "insertChild( parent = " + parent.value + ", child = " + value + " ; pre compare = " +
   parent.pre.compare( pre ) + "; post compare = " + parent.post.compare( post ))
      ...
      def insertRetroChild( parent: Vertex, value : A )( implicit tx: S#Tx ) : Vertex = {
      ...
      def insertRetroParent( child: Vertex, value : A )( implicit tx: S#Tx ) : Vertex = {
      ...
      private def addNewVertex( value: A, pre: preOrder.Entry, preTail: preOrder.Entry, post: postOrder.Entry )
...
//         val (t, treeSeq, parents) = randomlyFilledTree()
         ...
         @tailrec def testChild( version: Int, child: t.V ) {
            parents.get( child ) match {
               ...
               case Some( parent ) if( parent.version <= version ) =>
                  ...
                  val found = t.t.system.atomic { implicit tx => t.t.nearestNeighborOption( point, metric )}
   ...
   feature( "Marked ancestor lookup should be possible through isomorphic mapping between two quadtrees" ) {
      ...
      scenario( "Verifying marked ancestor lookup" ) {
         ...
         val mPreList   = {
            val res = SkipList.empty[ tm.preOrder.Entry ] // ( ord, m )
            res.add( tm.preOrder.root )
            res
         }
         val mPostList = {
            val res = SkipList.empty[ tm.postOrder.Entry ] // ( ord, m )
...
//println( " -- testing " + child )
            ...
            val found = tm.t.system.atomic { implicit tx => tm.t.nearestNeighborOption( point, metric ).map( _.value )}
            val parent = {
